...
This is the **handoff point** to the [`ember-polaris-migration`](../ember-polaris-migration) skill — Polaris-style authoring is now mainstream.
...
See the [`ember-polaris-migration`](../ember-polaris-migration) skill for the full workflow.
...
When all green, **switch to the modern Ember skills** for ongoing work — Polaris adoption, the modern addon skill, and the `ember-architect` / `ember-test-engineer` agents take over from here.

## See also

- `ember-4-recommendations` — the prerequisite work.
- [`ember-polaris-migration`](../ember-polaris-migration) — `<template>` tag adoption.
- [`ember-typescript-and-glint`](../ember-typescript-and-glint) — Glint setup at the destination.
